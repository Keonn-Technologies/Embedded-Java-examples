...
import com.keonn.adrd_150.ADRD_60_Series_SerialReader;
...
public class EmbeddedUtil implements ReadListener, ReaderIOListener, ReadExceptionListener{
	...
	private ADRD_60_Series_SerialReader reader;
	...
	private void run() {
		try {
			...
			reader = (ADRD_60_Series_SerialReader) tmReader;
			
			if(reader instanceof ADRD_60_Series_SerialReader){
				((ADRD_60_Series_SerialReader)reader).registerIOListener(this);
				io = ((ADRD_60_Series_SerialReader)reader).getGPIO();
			} else {
			...
			if(!enableGPI){
				...
				if(enableMux && (reader instanceof ADRD_60_Series_SerialReader)){
					System.out.println("Starting reader in autonomous mode to use muxes...");
					((ADRD_60_Series_SerialReader)reader).startFastMuxReading(fast_mux_antennas);
				} else {
			...
			try {
				...
				if(enableGPI){
					if(io.isLowToHigh()){
						...
						// stop RF
						if(enableMux && (reader instanceof ADRD_60_Series_SerialReader)){
							((ADRD_60_Series_SerialReader) reader).stopFastMuxReading();
						} else {
							reader.stopReading();
						}
						// send data
